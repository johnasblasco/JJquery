$(document).ready(function() {
    ...
    const data = [
        "angry", "bored", "calm", "confused", "crying",
        "funny", "happy", "loved", "nervous", "sad",
        "scared", "shocked", "shy", "sleepy", "tired",
    ];
...
const drop = (event, ui) => {
    ...
    if (draggedElementData === droppableElementData) {
        ...
        ui.draggable.remove(); 
        $(event.target).empty();
        $(event.target).append(`<img src="emotions/${draggedElementData}.png">`);
    ...
    const creator = () => {
        ...
        for (let i of randomData) {
            const emoDiv = $("<div>", {
                ...
                draggable: true,
                id: i
            }).html(`<img src="emotions/${i}.png">`);
            dragContainer.append(emoDiv);
        }

        randomData = randomData.sort(() => 0.5 - Math.random());
        for (let i of randomData) {
            const emotionsDiv = $("<div>", {
                class: "emotions",
                "data-id": i
            }).html(`${i.charAt(0).toUpperCase() + i.slice(1).replace("-", "")}`);
            dropContainer.append(emotionsDiv);
        }
